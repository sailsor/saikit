...
func (p *Protocer) execCmd() bool {
	...
	target := p.target + string(filepath.Separator) + p.packageName

	/*cmdLine := fmt.Sprintf("--go_out=plugins=grpc:%s --govalidators_out=%s"+
	" --proto_path %s %s"+
	" --proto_path=%s/src"+
	" --proto_path=%s/src/github.com/google/protobuf",
	target, target, p.protoPath, p.fromProto, goPath, goPath)*/
	...
	cmdLine := fmt.Sprintf(
		"--go_out=%s "+
			...
			"--proto_path=/usr/local/include/google/protobuf "+ //pb预定义类型
			"--proto_path %s %s",
		target, target, p.protoPath, p.fromProto)

	p.logger.Infof("target:%s;", target)
	p.logger.Infof("GOPATH:%s;", goPath)
	p.logger.Infof("proto path:%s;", p.protoPath)
	p.logger.Infof("from proto:%s;", p.fromProto)
